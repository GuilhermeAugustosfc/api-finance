...
def token(request, db: Session = Depends(get_db)):
    ...
    headers, body, status = oauth2_server.create_token_response(
        uri, request.method, request.body, request.headers
    )
...
def create_client(client_id: str, redirect_uri: str, db: Session = Depends(get_db)):
    ...
    if db_client:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Client already registered"
        )
...
def create_refresh_token(access_token: str, db: Session = Depends(get_db)):
    ...
    db_refresh_token = RefreshToken(
        token=refresh_token, expire_at=expire_at, access_token_id=db_access_token.id
    )
